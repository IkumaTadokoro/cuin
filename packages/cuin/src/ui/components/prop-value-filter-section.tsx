    ...
    <details class="group rounded border border-brand-200 transition">
      <summary class="flex cursor-pointer list-none items-center justify-between p-2 [&::-webkit-details-marker]:hidden">
        <div class="flex min-w-0 flex-1 items-center gap-1">
          <svg
            aria-hidden="true"
            class="h-3 w-3 shrink-0 -rotate-90 transition-transform group-open:rotate-0"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div class="rounded-b border-brand-200 border-t bg-white">
        <div class="border-brand-200 border-b p-2">
          <div class="relative">
            <SearchIcon class="absolute top-1/2 left-2 h-3 w-3 -translate-y-1/2 text-subtext-color" />
            <input
            ...
            <Show when={hasSearchQuery()}>
              <button
                class="absolute top-1/2 right-2 -translate-y-1/2 text-subtext-color hover:text-text-color"
                onClick={() => {
                  setSearchQuery("");
